      ...
      complex *16, allocatable :: sigma_nc(:),pot_nc(:),potex_nc(:)
      complex *16, allocatable :: sigma(:),pot(:),potex(:)
...
!      npatches = npatches_rhab
      ...
      allocate(sigma(npts),pot(npts),potex(npts))
      allocate(sigma_nc(npts_nc),pot_nc(npts_nc),potex_nc(npts_nc))

      call l3getsph(nmax,mm,nn,12,srcvals,sigma,npts,work)
      call l3getsph(nmax,mm,nn,12,srcvals_nc,sigma_nc,npts_nc,work)

      njh = nmax+ 5
      rscale = 1.0d0
      ifder = 1
      call besseljs3d(njh,zk,rscale,jvals,ifder,jder)
      call h3dall(njh,zk,rscale,hvals,ifder,hder)
      zfac_s = ima*zk*jvals(nn)*hvals(nn)
      zfac_d = ima*zk*zk*(jder(nn)*hvals(nn) + jvals(nn)*hder(nn))/2
      call prin2('zfac_s=*',zfac_s,2)
      call prin2('zfac_d=*',zfac_d,2)
      eps = 1.0d-8

      do i=1,npts
        potex(i) = sigma(i)*zfac_s
      enddo
      do i=1,npts_nc
        potex_nc(i) = sigma_nc(i)*zfac_s
      enddo

!
!
!  test single layer on conforming mesh
! 
      zpars(1) = zk
      zpars(2) = 1.0d0
      zpars(3) = 0.0d0
      ...
      erra_s = 0.0d0
      do i=1,npts
        erra_s = erra_s + abs(pot(i)-potex(i))**2*wts(i)
      enddo
      erra_s = sqrt(erra_s)
      erra_s_c = erra_s
      
!
! test single layer non-conforming mesh
!
!
      call lpcomp_helm_comb_dir(npatches_nc,norders_nc,ixyzs_nc, &
        iptype_nc,npts_nc,srccoefs_nc,srcvals_nc,12,npts_nc,srcvals_nc, &
        ipatch_id_nc,uvs_targ_nc,eps,zpars,sigma_nc,pot_nc)

      erra_s = 0.0d0
      do i=1,npts_nc
        erra_s = erra_s + abs(pot_nc(i)-potex_nc(i))**2*wts_nc(i)
      enddo
      erra_s_nc = sqrt(erra_s)



      do i=1,npts
        potex(i) = sigma(i)*zfac_d
      enddo
      do i=1,npts_nc
        potex_nc(i) = sigma_nc(i)*zfac_d
      enddo

!
!
!  test double layer on conforming mesh
! 
      zpars(1) = zk
      zpars(2) = 0.0d0
      zpars(3) = 1.0d0
      ...
      erra = 0.0d0
      do i=1,npts
        erra = erra + abs(pot(i)-potex(i))**2*wts(i)
      enddo
      erra_d_c = sqrt(erra)
      
!
! test double layer non-conforming mesh
!
!
      call lpcomp_helm_comb_dir(npatches_nc,norders_nc,ixyzs_nc, &
        iptype_nc,npts_nc,srccoefs_nc,srcvals_nc,12,npts_nc,srcvals_nc, &
      ...
      erra = 0.0d0
      do i=1,npts_nc
        erra = erra + abs(pot_nc(i)-potex_nc(i))**2*wts_nc(i)
      enddo
      erra_d_nc = sqrt(erra)


      call prin2('error in single layer evaluation=*',erra_s_c,1)
      call prin2('error in single layer evaluation non-conforming=*', &
        erra_s_nc,1)
      call prin2('error in double layer evaluation=*',erra_d_c,1)
      call prin2('error in double layer evaluation non-conforming=*', &
        erra_d_nc,1)
